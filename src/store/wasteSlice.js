...
const wasteSlice = createSlice({
  ...
  reducers: {
    ...
    updateWeight:     (state, action) => { const { id, weight } = action.payload; const item = state.basket.find(i => i.id === id); if (item) item.weight = weight },
    toggleSkip:       (state, action) => { const item = state.basket.find(i => i.id === action.payload); if (item) item.skipped = !item.skipped },
