  ...
  return (
    <main className="flex flex-col items-center px-4 py-10 gap-6">
        ...
        <div className="flex items-end gap-1 h-20">
          {WEEKLY.map((v, i) => (
            <div key={i} className="flex flex-col items-center gap-0.5 flex-1">
              <div
                style={{ height: `${(v / 100) * 64}px`, background: 'var(--green)', border: '1.5px solid var(--ink)' }}
                className="w-full"
              />
      ...
      <div className="w-full max-w-xl flex flex-col gap-3">
        <span className="font-data text-[12px] text-[var(--ink-2)] uppercase tracking-widest">{t.recentBookings}</span>
        {bookings.map(b => (
